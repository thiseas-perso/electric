      ...
      await new Promise((resolve, reject) => {
        fs.createReadStream(`${dataDirectory}/${file}`)
          ...
          .on('data', function (row) {
            ...
            const modelID = model.split(' ').join('-').toLowerCase();
            const version =
              modelEnd === carNameLength
                ? 'base'
...
export const getAllFullTestedVersions = async () => {
  const data = await getAllData();
  return data.reduce((acc, car) => {
    const filtered = car.versions.filter(
      (version) =>
        version.hasOwnProperty('RANGE') &&
        version.hasOwnProperty('ACCELERATION')
    );
    if (filtered.length) {
